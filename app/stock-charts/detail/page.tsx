...
export default function StockDetailPage() {
  ...
  useEffect(() => {
    ...
    const endMonth   = selectedYear === currentYear
      ...
      : `${selectedYear}-12`
    fetch(`/api/pension-eval/range?sessionId=${stockSid}&startMonth=${startMonth}&endMonth=${endMonth}`)
      .then(r => r.json())
      .then(d => { setMonthlyData(d.monthlyData ?? []); setAssetOrder(d.assetOrder ?? []) })
      .finally(() => setLoading(false))
  }, [stockSid, selectedYear])
                ...
                <div className="px-3 pt-3 pb-0">
                  <ResponsiveContainer width="100%" height={210}>
                    <ComposedChart data={chartData} margin={{ top: 10, right: 42, bottom: 5, left: 0 }}>
                      ...
                      <Bar yAxisId="left" dataKey="매수금액"  name="매수금액"  stackId="a" fill="#bbf7d0" maxBarSize={28} />
                  ...
                  <div className="flex items-center gap-4 justify-center pb-3 text-[10px] text-gray-400">
                    <span className="flex items-center gap-1.5">
                      <span className="w-3 h-2.5 rounded-sm bg-[#bbf7d0] inline-block" />매수금액
                    </span>
                    <span className="flex items-center gap-1.5">
                    ...
                    <span className="flex items-center gap-1.5">
                      <span className="w-4 h-0.5 bg-[#16a34a] inline-block rounded" />수익률
                    </span>
                  </div>
                </div>
