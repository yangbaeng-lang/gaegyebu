...
function ScrollChart({ data, children }: { data: any[]; children: (w: number, h: number) => React.ReactNode }) {
  ...
  return (
    <>
      <style>{`
        .sd-scroll::-webkit-scrollbar{height:4px}
        .sd-scroll::-webkit-scrollbar-track{background:#f1f5f9;border-radius:4px}
        .sd-scroll::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:4px;min-width:24px}
        .sd-scroll::-webkit-scrollbar-thumb:hover{background:#94a3b8}
      `}</style>
      <div
        ref={wrapRef}
        className="sd-scroll overflow-x-auto overflow-y-hidden h-full select-none"
    ...
    <div className="flex rounded-lg overflow-hidden border border-gray-200 text-xs font-semibold flex-shrink-0">
      {(['monthly', 'yearly'] as const).map(m => (
        <button key={m} onClick={() => onChange(m)}
          className={`px-3 py-1.5 transition-colors ${mode === m ? 'bg-[#22c55e] text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>
          {m === 'monthly' ? '월별' : '연도별'}
        </button>
      ))}
    </div>
...
export default function StockDetailPage() {
  ...
  useEffect(() => {
    fetch('/api/sessions').then(r => r.json()).then(d => {
      ...
      setStockSid(list.find(s => s.name === '주식')?.id ?? null)
    })
  }, [])
  ...
  useEffect(() => {
    ...
    fetch(`/api/pension-eval/range?sessionId=${stockSid}&startMonth=2020-01&endMonth=${endMonth}`)
      .then(r => r.json())
              ...
              <div key={name} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="px-4 sm:px-5 pt-3 sm:pt-4 pb-2 sm:pb-3 border-b border-gray-50">
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex items-center gap-2 min-w-0">
                      <span className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ backgroundColor: color || '#22c55e' }} />
                  ...
                  <div className="flex gap-4 mt-1.5 text-xs">
                    <span className="text-gray-400">매수 <span className="font-semibold text-gray-700">{fmt(납입액)}</span></span>
                    {평가손익 !== null && (
                  ...
                  <ScrollChart data={chartData}>
                    {(w, h) => (
                      <ComposedChart data={chartData} width={w} height={h} margin={{ top: 10, right: 36, bottom: 5, left: 0 }}>
                        ...
                        {/* 누적 막대: 매수금액(하단) + 평가손익(상단) */}
                        <Bar yAxisId="left" dataKey="매수금액"  name="매수금액"  stackId="a" fill="#fecaca" maxBarSize={26} />
                        <Bar yAxisId="left" dataKey="평가손익" name="평가손익" stackId="a" maxBarSize={26}>
                          {chartData.map((entry, i) => (
                            <Cell key={i} fill={(entry.평가손익 ?? 0) >= 0 ? '#f87171' : '#93c5fd'} />
                          ))}
                        </Bar>
                ...
                <div className="flex items-center gap-3 sm:gap-4 justify-center py-2 text-[10px] text-gray-400">
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-2.5 rounded-sm bg-[#fecaca] inline-block" />매수금액
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-2.5 rounded-sm bg-[#f87171] inline-block" />평가손익(+)
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-2.5 rounded-sm bg-[#93c5fd] inline-block" />평가손익(−)
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-4 h-0.5 bg-[#ef4444] inline-block rounded" />수익률
                  </span>
                </div>
