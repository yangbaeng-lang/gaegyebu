...
export default function PeriodNav({ viewMode, setViewMode, label, year, setYear, quarter, setQuarter, month, setMonth, prev, next, children }: Props) {
  ...
  return (
    <div className="relative bg-white border-b border-gray-100 px-5 h-[60px] flex items-center justify-center gap-2 flex-shrink-0">

      {/* 모드 탭 */}
      <div ref={tabsRef} className="flex gap-0.5 bg-gray-100 rounded-lg p-0.5">
        {(['year', 'quarter', 'month'] as const).map(m => (
          <button key={m} onClick={() => setViewMode(m)}
            className={`text-[14px] px-2.5 py-0.5 rounded-md transition-all
              ${viewMode === m ? 'bg-white text-gray-800 shadow-sm font-medium' : 'text-gray-400 hover:text-gray-600'}`}>
            {m === 'year' ? '연도별' : m === 'quarter' ? '분기별' : '월별'}
          </button>
        ))}
      </div>

      {/* 이전 버튼 */}
      <button onClick={prev} className="w-8 h-8 flex items-center justify-center border border-gray-200 rounded-lg text-gray-400 hover:bg-gray-50 hover:text-gray-700 transition-colors">
      ...
      <div ref={pickerRef} className="relative">
        <button onClick={handleOpen}
          className="text-[14px] font-medium text-gray-700 min-w-[110px] text-center px-2 py-1 rounded-lg hover:bg-gray-100 transition-colors">
          {label}
        </button>
      ...
      {/* 다음 버튼 */}
      <button onClick={next} className="w-8 h-8 flex items-center justify-center border border-gray-200 rounded-lg text-gray-400 hover:bg-gray-50 hover:text-gray-700 transition-colors">
        <i className="ti ti-chevron-right text-base" />
      </button>

      {/* 탭 너비만큼 투명 spacer → 레이블 정중앙 정렬 */}
      <div style={{ width: tabsWidth }} className="flex-shrink-0" />
